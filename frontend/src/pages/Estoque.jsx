...
import { Package, AlertTriangle, Search, Settings, CheckCircle, Plus, Trash2 } from 'lucide-react'
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="card text-center">
          ...
          <p className="text-2xl font-bold text-green-600">{itens.filter(i => !i.alerta).length}</p>
          <p className="text-xs text-gray-500">Itens Normais</p>
        </div>
        <div className="card text-center">
          <AlertTriangle size={24} className="mx-auto text-red-500 mb-2" />
          <p className="text-2xl font-bold text-red-600">{itens.filter(i => i.alerta).length}</p>
      ...
      <div className="card">
        {/* Abas */}
        <div className="flex gap-1 mb-4 border-b border-gray-100 pb-3 flex-wrap">
        ...
        {aba === 'itens' && (
          <>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100">
                    ...
                    <th className="text-left py-2 px-3 text-xs font-semibold text-gray-500">Depósito</th>
                    <th className="text-right py-2 px-3 text-xs font-semibold text-gray-500">Quantidade</th>
                    <th className="text-right py-2 px-3 text-xs font-semibold text-gray-500">Mínimo</th>
                ...
                <tbody>
                  {itensFiltrados.slice(0, 100).map((item, i) => (
                    <tr key={i} className={`border-b border-gray-50 hover:bg-gray-50 ${item.alerta ? 'bg-red-50' : ''}`}>
                      <td className="py-2 px-3 font-mono text-xs">{item.codigo}</td>
                      <td className="py-2 px-3 text-gray-700">{item.descricao}</td>
                      <td className="py-2 px-3 text-gray-500 text-xs">{item.deposito}</td>
                      <td className="py-2 px-3 text-right font-semibold">{Number(item.quantidade || 0).toLocaleString('pt-BR')} {item.unidade}</td>
                      <td className="py-2 px-3 text-right text-gray-500">{item.estoque_minimo != null ? Number(item.estoque_minimo).toLocaleString('pt-BR') : '—'}</td>
                      <td className="py-2 px-3 text-center">
                        {item.alerta ? <span className="badge-vermelho">Crítico</span> : <span className="badge-verde">Normal</span>}
                      </td>
                      <td className="py-2 px-3 text-center">
                        <button onClick={() => removerItem(item)} className="text-red-500 hover:text-red-700">
                          <Trash2 size={14} />
                        </button>
                      </td>
                    </tr>
              ...
              {itensFiltrados.length === 0 && <p className="text-center text-gray-400 py-8">Nenhum item encontrado. Cadastre um item na aba "Novo Item" ou importe uma planilha na Administração.</p>}
            </div>
          </>
        )}
        ...
        {aba === 'novo' && (
          <form onSubmit={salvarNovoItem} className="max-w-2xl space-y-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-sm font-medium text-gray-700">Cadastrar novo item de estoque</p>
            </div>
            ...
            {msgItem && <p className={`text-sm ${msgItem.includes('Erro') || msgItem.includes('já existe') ? 'text-red-600' : 'text-green-600'}`}>{msgItem}</p>}
          ...
          <div className="space-y-3">
            {alertas.length === 0 && (
              <div className="text-center py-10">
                ...
                <p className="text-gray-500 font-medium">Nenhum alerta pendente</p>
                <p className="text-xs text-gray-400 mt-1">Todos os produtos estão acima do estoque mínimo</p>
              </div>
            )}
            {alertas.map(a => (
              <div key={a.id} className="flex items-center gap-4 bg-amber-50 border border-amber-200 rounded-xl p-4">
                <AlertTriangle size={20} className="text-amber-500 flex-shrink-0" />
                <div className="flex-1">
                  <p className="font-semibold text-gray-800 text-sm">{a.produto_descricao}</p>
                  <p className="text-xs text-gray-500">Código: {a.produto_codigo}</p>
        ...
        {aba === 'configurar' && (
          <form onSubmit={salvarConfig} className="max-w-md space-y-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-sm font-medium text-gray-700">Configurar estoque mínimo por produto</p>
            </div>
            <div>
            ...
            <p className="text-xs text-gray-400">Dica: o produto precisa estar cadastrado na aba "Novo Item" (ou importado por planilha) antes de configurar o mínimo.</p>
            {msg && <p className={`text-sm ${msg.includes('Erro') || msg.includes('não encontrado') ? 'text-red-600' : 'text-green-600'}`}>{msg}</p>}
