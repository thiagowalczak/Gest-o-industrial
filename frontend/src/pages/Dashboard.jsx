...
import {
  ...
  ShoppingCart, AlertTriangle, RefreshCw, Wifi, BarChart3
} from 'lucide-react'
    ...
    <div className="card">
      <div className="flex items-start justify-between">
        <div className="min-w-0">
          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">{titulo}</p>
          <p className={`text-lg sm:text-2xl font-bold mt-1 ${cor}`}>{valor}</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">Painel Central</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {atualizado ? `Atualizado às ${atualizado.toLocaleTimeString('pt-BR')}` : 'Carregando...'}
          </p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400">
            <Wifi size={12} />
            Sistema Online
          </div>
      ...
      {(dados?.alertas_estoque > 0) && (
        <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 dark:bg-amber-900/20 dark:border-amber-800 rounded-xl px-4 py-3">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="card">
            <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Financeiro — próximos 30 dias</h4>
            {dados ? (
              <ComparativoBarras
                dados={[
                  { nome: 'A Receber', valor: fin.total_receber_30d || 0, cor: '#22c55e' },
                  { nome: 'A Pagar', valor: fin.total_pagar_30d || 0, cor: '#ef4444' },
                ]}
                formatador={fmt}
              />
            ) : (
          ...
          <div className="card">
            <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Produção — previsto x produzido</h4>
            {dados ? (
              <ComparativoBarras
                dados={[
                  { nome: 'Previsto', valor: prod.quantidade_prevista || 0, cor: '#a855f7' },
                  { nome: 'Produzido', valor: prod.quantidade_produzida || 0, cor: '#6366f1' },
                ]}
                formatador={fmtNum}
              />
            ) : (
