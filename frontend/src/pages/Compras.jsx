...
export default function Compras() {
  ...
  const remover = async (p) => {
    if (!confirm(`Remover o pedido ${p.numero}/${p.item}?`)) return
    try {
      await api.delete(`/producao/compras/${p.id}`)
      carregar()
    } catch {}
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="card flex items-center justify-between flex-wrap gap-3">
        <div>
          <p className="text-sm font-semibold text-gray-800">Pedidos de Compra</p>
      ...
      <div className="card">
        ...
        {mostrarForm && (
          <form onSubmit={salvar} className="bg-gray-50 border border-gray-100 rounded-xl p-4 mb-4 space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-gray-700">{editando ? `Editar pedido ${editando.numero}/${editando.item}` : 'Novo pedido de compra'}</p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="text-left py-2 px-3 text-xs font-semibold text-gray-500">Entrega</th>
            ...
            <tbody>
              {pedidos.map((p, i) => {
                ...
                return (
                  <tr key={i} className={`border-b border-gray-50 hover:bg-gray-50 ${atrasado ? 'bg-red-50' : ''}`}>
                    ...
                    <td className="py-2 px-3 text-xs text-gray-500 truncate max-w-[150px]">{p.nome_fornecedor || p.fornecedor}</td>
                    ...
                    <td className={`py-2 px-3 font-mono text-xs ${atrasado ? 'text-red-600 font-bold' : 'text-gray-500'}`}>
                    ...
                    <td className="py-2 px-3">
                      <div className="flex items-center justify-center gap-2">
                        <button onClick={() => abrirEdicao(p)} className="text-gray-400 hover:text-primary-600"><Edit2 size={14} /></button>
                        <button onClick={() => remover(p)} className="text-gray-400 hover:text-red-600"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                )
          ...
          {pedidos.length === 0 && <p className="text-center text-gray-400 py-8">Nenhum pedido cadastrado. Cadastre um novo pedido ou importe pela Administração.</p>}
        </div>
      </div>
    </div>
  )
